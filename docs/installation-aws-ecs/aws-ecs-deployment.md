...
```bash
# in ~/<your-path-to-cloned-repo>/amundsenfrontendlibrary/docs/instalation-aws-ecs
$ ecs-cli configure --cluster optimusddc --region us-west-2 --default-launch-type EC2 --config-name optimusddc
```

### STEP 2: Create a profile using your access key and secret key:

```bash
# in ~/<your-path-to-cloned-repo>/optimusddc/docs/installation-aws-ecs
$ ecs-cli configure profile --access-key $AWS_ACCESS_KEY_ID --secret-key $AWS_SECRET_ACCESS_KEY --profile-name optimusddc
```
...
```bash
# in ~/<your-path-to-cloned-repo>/optimusddc/docs/installation-aws-ecs
$ ecs-cli up --keypair JoaoCorreia --extra-user-data userData.sh --capability-iam --size 1 --instance-type t2.large --cluster-config optimusddc --verbose --force --aws-profile $AWS_PROFILE
```
...
```bash
# in ~/<your-path-to-cloned-repo>/optimusddc/docs/installation-aws-ecs
$ ecs-cli compose --cluster-config optimusddc --file docker-ecs-optimusddc.yml up --create-log-groups
```
