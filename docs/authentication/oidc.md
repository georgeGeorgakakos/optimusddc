...
- You are using k8s to setup amundsen. See [amundsen-kube-helm](../../optimusddc-kube-helm/README.md)
...
3. At present, there is no oidc build of the frontend. So you will need to build an oidc build yourself and upload it to, for example ECR, for use by k8s.
   ...
   ```yaml
   frontEndServiceImage: 123.dkr.ecr.us-west-2.amazonaws.com/edmunds/optimusddc-frontend:oidc-test
   ```
