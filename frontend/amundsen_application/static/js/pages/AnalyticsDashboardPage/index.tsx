...
type TabId = 'overview' | 'logging' | 'ems-logs' | 'events' | 'performance' | 'stores' | 'peers' | 'credentials' | 'metadata';
...
const AnalyticsDashboardPage: React.FC = () => {
  ...
  const [logDetailEntry, setLogDetailEntry] = useState<LoggerEntry | null>(null);
  ...
  const [catalogStats, setCatalogStats] = useState<{ total: number; tables: number; updated: string } | null>(null);
  const [toscaStats, setToscaStats] = useState<{ files: number; size: number; uploaders: number; last: string } | null>(null);
  ...
  const [credDetailEntry, setCredDetailEntry] = useState<CredentialEntry | null>(null);

  // ── Metadata tab ──
  const [metadataMetrics, setMetadataMetrics] = useState<MetadataMetrics | null>(null);
  const [metadataHealth, setMetadataHealth] = useState<MetadataHealth | null>(null);
    ...
    try {
      ...
      await Promise.all(
        nodes.map(async (node) => {
          try {
            const url = buildApiUrl('optimusdb', `/${CONTEXT}/agent/status`, node.id);
  ...
  const fetchLoggerData = useCallback(async (nodeId: number = selectedAgent) => {
    ...
    try {
      ...
      entries = entries.filter((e) => e && e.timestamp).map((e, idx) => ({
        date: e.date || '',
        hour: e.hour || '',
        id: e.id || idx + 1,
        level: e.level || 'INFO',
  ...
  const fetchEmsLogs = useCallback(async (nodeId: number = selectedAgent) => {
  ...
  // ── Fetch EMS events ──
  const fetchEmsEvents = useCallback(async (nodeId: number = selectedAgent) => {
  ...
  const fetchPerformanceData = useCallback(async (nodeId: number = selectedAgent) => {
    setPerfLoading(true);
    try {
      // Benchmarks
      const bmUrl = buildApiUrl('optimusdb', `/${CONTEXT}/benchmarks`, nodeId);
      const bmResp = await axios.get(bmUrl, { timeout: 8000 }).catch(() => null);
      ...
      const cacheUrl = buildApiUrl('optimusdb', `/${CONTEXT}/command`, nodeId);
      const cacheResp = await axios.post(cacheUrl, { command: 'cache_stats' }, { timeout: 5000 }).catch(() => null);
      ...
      if (cacheResp?.data) {
        ...
        setCacheStats({
          hits: c.hits || 0,
          misses: c.misses || 0,
          hit_ratio: c.hit_ratio || (c.hits && c.misses ? c.hits / (c.hits + c.misses) : 0),
  ...
  const fetchStoresData = useCallback(async () => {
    ...
    try {
      ...
      await Promise.all(
        nodes.map(async (node) => {
          try {
            const url = buildApiUrl('optimusdb', `/${CONTEXT}/debug/optimusdb/mesh`, node.id);
            const resp = await axios.get(url, { timeout: 5000 });
            const d = resp.data;

            let stores: Record<string, MeshStoreInfo> = {};
            if (d.orbitdb_stores) {
              ...
              if (Array.isArray(d.orbitdb_stores)) {
                ...
                Object.entries(d.orbitdb_stores).forEach(([name, info]: [string, any]) => {
                  stores[name] = {
            ...
            meshResults.push({ nodeId: node.id, nodeName: node.name || `OptimusDB-${node.id}`, stores: {} });
          }
        })
      );
      ...
      // Fetch catalog stats via SQL
      try {
        const catUrl = buildApiUrl('optimusdb', `/${CONTEXT}/ems/sql`, selectedAgent);
        const catResp = await axios.get(catUrl, {
          params: { q: 'SELECT * FROM datacatalog' },
          timeout: 8000,
        });
        const catData = Array.isArray(catResp.data) ? catResp.data : (catResp.data?.results || catResp.data?.data || []);
        const tables = new Set(catData.map((r: any) => r.table_name || r.tableName).filter(Boolean));
        setCatalogStats({
          total: catData.length,
          tables: tables.size,
          updated: catData.length > 0 ? (catData[0].timestamp || catData[0].updated_at || '') : '',
        });
      } catch { setCatalogStats(null); }

      // Fetch TOSCA stats via SQL
      try {
        const toscaUrl = buildApiUrl('optimusdb', `/${CONTEXT}/ems/sql`, selectedAgent);
        const toscaResp = await axios.get(toscaUrl, {
          params: { q: 'SELECT * FROM toscametadata' },
          timeout: 8000,
        });
        const toscaData = Array.isArray(toscaResp.data) ? toscaResp.data : (toscaResp.data?.results || toscaResp.data?.data || []);
        const uploaders = new Set(toscaData.map((r: any) => r.uploader || r.uploaded_by).filter(Boolean));
        const totalSize = toscaData.reduce((s: number, r: any) => s + (r.size || r.file_size || 0), 0);
        setToscaStats({
          files: toscaData.length,
          size: totalSize,
          uploaders: uploaders.size,
          last: toscaData.length > 0 ? (toscaData[0].timestamp || toscaData[0].uploaded_at || '') : '',
        });
      } catch { setToscaStats(null); }
    } catch (err) {
    ...
    try {
      ...
      await Promise.all(
        nodes.map(async (node) => {
          try {
            // Fetch peers
            const peersUrl = buildApiUrl('optimusdb', `/${CONTEXT}/peers`, node.id);
            ...
            const statusUrl = buildApiUrl('optimusdb', `/${CONTEXT}/agent/status`, node.id);
            ...
            peerResults.push({
              ...
              peers: peersList.map((p: any) => ({
                ...
                addrs: Array.isArray(p.addrs) ? p.addrs : (p.address ? [p.address] : []),
            ...
            // Fetch contributions
            try {
              const contriUrl = buildApiUrl('optimusdb', `/${CONTEXT}/command`, node.id);
              const contriResp = await axios.post(contriUrl, {
            ...
            } catch { /* contribution data optional */ }
          } catch (err) {
            console.warn(`Failed to fetch peer data from node ${node.id}:`, err);
          }
        })
      );
  ...
  const fetchCredentials = useCallback(async () => {
    ...
    try {
      ...
      const credList: CredentialEntry[] = (Array.isArray(rawCreds) ? rawCreds : Array.isArray(rawCreds?.credentials) ? rawCreds.credentials : [])
        .map((c: any) => ({
          id: c.id || c['@id'] || '',
          type: Array.isArray(c.type) ? c.type : [c.type || 'VerifiableCredential'],
  ...
  const fetchMetadataData = useCallback(async () => {
    ...
    try {
      ...
      // Fetch metrics
      try {
        const metricsUrl = buildApiUrl('optimusdb', `/api/v1/metadata/metrics`, nodeId);
        ...
        setMetadataMetrics({
          ...
          cache_hit_ratio: m.cache_hit_ratio || (m.cache_hits && m.cache_misses ? m.cache_hits / (m.cache_hits + m.cache_misses) : 0),
      ...
      } catch { setMetadataMetrics(null); }

      // Fetch health
      try {
        const healthUrl = buildApiUrl('optimusdb', `/api/v1/metadata/health`, nodeId);
  ...
  }, [activeTab, fetchClusterStatus, fetchLoggerData, fetchEmsLogs, fetchEmsEvents, fetchPerformanceData, fetchStoresData, fetchPeerData, fetchCredentials, fetchMetadataData]);
  ...
  const allLogLevels = useMemo(() => [...new Set(loggerData.map((l) => l.level))].sort(), [loggerData]);
  const allLogSourceFiles = useMemo(() => [...new Set(loggerData.map((l) => l.source.split(':')[0]).filter(Boolean))].sort(), [loggerData]);
  const allLogHours = useMemo(() => [...new Set(loggerData.map((l) => l.hour).filter(Boolean))].sort(), [loggerData]);

  const logLevelCounts = useMemo(() => {
    const c: Record<string, number> = {};
    loggerData.forEach((l) => { c[l.level] = (c[l.level] || 0) + 1; });
    return c;
  }, [loggerData]);

  const filteredLoggerData = useMemo(() => {
    let data = [...loggerData];
    if (logLevelFilter.size > 0) data = data.filter((l) => logLevelFilter.has(l.level));
    if (logHourFilter !== 'ALL') data = data.filter((l) => l.hour === logHourFilter);
    if (logSourceFilter) data = data.filter((l) => l.source.toLowerCase().includes(logSourceFilter.toLowerCase()));
    if (logSearchTerm) {
    ...
    data.sort((a, b) => {
      ...
      else cmp = String((a as any)[logSortField]).localeCompare(String((b as any)[logSortField]));
      return logSortDir === 'desc' ? -cmp : cmp;
    });
    return data;
  }, [loggerData, logLevelFilter, logHourFilter, logSourceFilter, logSearchTerm, logSortField, logSortDir]);

  const filteredLogLevelCounts = useMemo(() => {
    const c: Record<string, number> = {};
    filteredLoggerData.forEach((l) => { c[l.level] = (c[l.level] || 0) + 1; });
    return c;
  }, [filteredLoggerData]);

  const logTotalPages = Math.max(1, Math.ceil(filteredLoggerData.length / logPageSize));
  const pagedLogData = filteredLoggerData.slice((logPage - 1) * logPageSize, logPage * logPageSize);
  const hasLogFilters = logLevelFilter.size > 0 || logSourceFilter !== '' || logSearchTerm !== '' || logHourFilter !== 'ALL';
  ...
  const exportLogData = (format: 'csv' | 'json') => {
    ...
    if (format === 'json') {
      ...
      const csv = [
        ...
        ...data.map((r) => `${r.id},"${r.date}","${r.hour}","${r.timestamp}","${r.level}","${r.source}","${r.message.replace(/"/g, '""')}"`)
      ].join('\n');
  ...
  const emsLogLevelCounts = useMemo(() => {
    const c: Record<string, number> = {};
    emsLogs.forEach((l) => { c[l.level] = (c[l.level] || 0) + 1; });
    return c;
  }, [emsLogs]);
  ...
  const avgHealth = agents.length > 0 ? (agents.reduce((s, a) => s + a.health, 0) / agents.length).toFixed(1) : '0';
  ...
  const filteredCreds = useMemo(() => {
    return credentials.filter((c) => {
      if (credTypeFilter !== 'ALL' && !c.type.includes(credTypeFilter)) return false;
      if (credSearchTerm) {
  ...
  const credIssuers = useMemo(() => new Set(credentials.map((c) => c.issuer)), [credentials]);
  const credSubjects = useMemo(() => new Set(credentials.map((c) => c.credentialSubject?.id).filter(Boolean)), [credentials]);
    ...
    <DocumentTitle title="Analytics Dashboard - OptimusDDC">
      <div className="adp-page">
          ...
          <div className="adp-tb-right">
            <div className="adp-agent-select">
              ...
              <select value={selectedAgent} onChange={(e) => setSelectedAgent(Number(e.target.value))}>
                {(agents.length > 0 ? agents : [{ nodeId: 1, name: 'optimusdb1' }, { nodeId: 2, name: 'optimusdb2' }, { nodeId: 3, name: 'optimusdb3' }] as any[]).map((a) => (
            ...
            <button
              className={`adp-btn-ghost ${autoRefresh ? 'adp-active-refresh' : ''}`}
              onClick={() => setAutoRefresh(!autoRefresh)}
            >
              {autoRefresh ? '🔄 Live' : '⏸ Paused'} <span className="adp-muted">30s</span>
            </button>
            <button className="adp-btn-ghost" onClick={refreshAll} disabled={loading}>
              {loading ? '⏳' : '🔄'}
            </button>
          </div>
        </div>
        ...
        <div className="adp-content">
          ...
          {activeTab === 'overview' && (
            <div className="adp-tab-body">
              ...
              <div className="adp-kpi-row">
                {[
                  { label: 'Cluster Health', value: `${avgHealth}%`, sub: `${healthyCount}/${agents.length} nodes`, color: '#16a34a', icon: '💚' },
                  { label: 'Active Nodes', value: `${healthyCount} / ${agents.length}`, sub: leader ? `Leader: ${leader.name}` : 'No leader', color: '#2563eb', icon: '🖥️' },
                  { label: 'Election Term', value: leader ? `#${leader.electionTerm}` : 'N/A', sub: 'Leader stable', color: '#8b5cf6', icon: '🗳️' },
                  { label: 'Logger Entries', value: String(loggerData.length), sub: `${logLevelCounts.ERROR || 0} errors`, color: '#667eea', icon: '📋' },
                  { label: 'EMS Events', value: String(emsEvents.length), sub: 'Last hour', color: '#0d9488', icon: '⚡' },
                  { label: 'Cache Hit Ratio', value: cacheStats ? `${(cacheStats.hit_ratio * 100).toFixed(1)}%` : 'N/A', sub: cacheStats ? `${cacheStats.hits} hits` : '', color: '#d97706', icon: '🎯' },
                ].map((m, i) => (
                  <div key={i} className="adp-kpi-card">
                    ...
                    <div className="adp-kpi-value" style={{ color: m.color }}>{m.value}</div>
              ...
              <div className="adp-card">
                <div className="adp-card-header">
                  <h3 className="adp-card-title">🖥️ Node Status <span className="adp-card-src">from /agent/status</span></h3>
                </div>
                <div className="adp-node-grid">
                  {agents.map((a) => (
                    <div key={a.nodeId} className="adp-node-card">
                      <div className="adp-node-top">
                        <span className={`adp-node-dot ${a.health > 0 ? 'healthy' : 'down'}`} />
                        <span className="adp-node-name">{a.name}</span>
                        <span className={`adp-badge ${a.isLeader ? 'leader' : 'follower'}`}>
                          {a.isLeader ? '⭐ Leader' : 'Follower'}
                        </span>
                      </div>
                      <div className="adp-node-stats">
                        <div><span className="adp-muted">Health:</span> <strong className={a.health > 50 ? 'adp-green' : 'adp-amber'}>{a.health}%</strong></div>
                        <div><span className="adp-muted">CPU:</span> <strong>{a.cpu}</strong></div>
                        <div><span className="adp-muted">Memory:</span> <strong>{a.memory}</strong></div>
                        <div><span className="adp-muted">Uptime:</span> <strong>{a.uptime}</strong></div>
                      </div>
                      <div className="adp-health-bar">
                        <div className="adp-health-fill" style={{ width: `${a.health}%`, background: a.health > 50 ? '#16a34a' : '#d97706' }} />
                      </div>
                    </div>
                  ))}
                  {agents.length === 0 && (
              ...
              <div className="adp-two-col">
                <div className="adp-card">
                  <div className="adp-card-header">
                    ...
                    <button className="adp-btn-sm" onClick={() => setActiveTab('logging')}>View All →</button>
                  </div>
                  <div className="adp-mini-list">
                    {loggerData.slice(0, 8).map((log, i) => (
                      <div key={i} className="adp-mini-row">
                        <span className="adp-level-badge" style={{ background: `${getLevelColor(log.level)}15`, color: getLevelColor(log.level) }}>{log.level}</span>
                        <span className="adp-mini-msg">{log.message}</span>
                        <span className="adp-mini-time">{fmtTime(log.timestamp)}</span>
                      </div>
                    ))}
                    {loggerData.length === 0 && <div className="adp-empty">No log data yet</div>}
                  </div>
                </div>
                <div className="adp-card">
                  <div className="adp-card-header">
                    <h3 className="adp-card-title">⚡ Recent Events</h3>
                    <button className="adp-btn-sm" onClick={() => setActiveTab('events')}>View All →</button>
                  </div>
                  <div className="adp-mini-list">
                    {emsEvents.slice(0, 8).map((ev, i) => (
                      <div key={i} className="adp-mini-row">
                        <span className="adp-level-badge" style={{ background: `${getEventColor(ev.type)}15`, color: getEventColor(ev.type) }}>{ev.type}</span>
                        <span className="adp-mini-msg">{ev.detail}</span>
                        <span className="adp-mini-time">{fmtTime(ev.timestamp)}</span>
                      </div>
                    ))}
                    {emsEvents.length === 0 && <div className="adp-empty">No events yet</div>}
                  </div>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'logging' && (
            <div className="adp-tab-body">
              ...
              <div className="adp-kpi-row compact">
                {[
                  { l: 'Total Entries', v: loggerData.length, c: '#667eea', i: '📊' },
                  { l: 'Filtered', v: filteredLoggerData.length, c: '#2563eb', i: '🔍' },
                  { l: 'Errors', v: logLevelCounts.ERROR || 0, c: '#dc2626', i: '❌' },
                  { l: 'Warnings', v: logLevelCounts.WARN || 0, c: '#d97706', i: '⚠️' },
                  { l: 'Elections', v: logLevelCounts.ELECTION || 0, c: '#8b5cf6', i: '🗳️' },
                  { l: 'Queries', v: logLevelCounts.QUERY || 0, c: '#2563eb', i: '🔎' },
                ].map((m, i) => (
                  <div key={i} className="adp-kpi-card compact">
                    <div className="adp-kpi-header"><span className="adp-kpi-label">{m.l}</span><span className="adp-kpi-icon">{m.i}</span></div>
                    <div className="adp-kpi-value" style={{ color: m.c }}>{m.v}</div>
                  </div>
                ))}
              </div>
              ...
              <div className="adp-card adp-log-filters">
                <div className="adp-filter-row">
                  ...
                  {allLogLevels.map((lv) => {
                    ...
                    return (
                      <button key={lv} className={`adp-chip ${on ? 'active' : ''}`}
                              style={on ? { borderColor: c, background: `${c}15`, color: c } : {}}
                              onClick={() => toggleLogLevel(lv)}>
                        {lv} <span className="adp-chip-count">({logLevelCounts[lv] || 0})</span>
                      </button>
                    );
                  })}
                  {hasLogFilters && (
                    <button className="adp-chip adp-chip-clear" onClick={clearLogFilters}>✕ Clear All</button>
                  )}
                </div>

                <div className="adp-filter-row">
                  <input type="text" className="adp-search-input" placeholder="🔍 Search messages, sources, levels..."
                         value={logSearchTerm} onChange={(e) => { setLogSearchTerm(e.target.value); setLogPage(1); }} />

                  <span className="adp-filter-label">Source:</span>
                  <select className="adp-select" value={logSourceFilter} onChange={(e) => { setLogSourceFilter(e.target.value); setLogPage(1); }}>
                    <option value="">All Sources</option>
                    {allLogSourceFiles.map((s) => <option key={s} value={s}>{s}</option>)}
                  </select>

                  <span className="adp-filter-label">Hour:</span>
                  <div className="adp-btn-group">
                    <button className={logHourFilter === 'ALL' ? 'active' : ''} onClick={() => { setLogHourFilter('ALL'); setLogPage(1); }}>All</button>
                    {allLogHours.map((h) => (
                      <button key={h} className={logHourFilter === h ? 'active' : ''} onClick={() => { setLogHourFilter(h); setLogPage(1); }}>{h}:00</button>
                    ))}
                  </div>

                  <div className="adp-filter-sep" />
                  <button className="adp-btn-sm" onClick={() => exportLogData('csv')}>📥 CSV</button>
                  <button className="adp-btn-sm" onClick={() => exportLogData('json')}>📥 JSON</button>
                </div>
              </div>
              ...
              <div className="adp-card adp-distrib-bar">
                <div className="adp-distrib-track">
                  {allLogLevels.map((lv) => {
                    const pct = filteredLoggerData.length > 0 ? ((filteredLogLevelCounts[lv] || 0) / filteredLoggerData.length * 100) : 0;
                    return pct > 0 ? (
                      <div key={lv} className="adp-distrib-seg" title={`${lv}: ${filteredLogLevelCounts[lv]} (${pct.toFixed(1)}%)`}
                           style={{ width: `${pct}%`, background: getLevelColor(lv) }} />
                    ) : null;
                  })}
                </div>
                  ...
                  {allLogLevels.map((lv) => {
                    ...
                    return c > 0 ? (
                      <span key={lv} className="adp-legend-item">
                        <span className="adp-legend-dot" style={{ background: getLevelColor(lv) }} />
                        {lv}: <strong>{c}</strong>
                      </span>
                    ) : null;
                  })}
              ...
              <div className="adp-card">
                <div className="adp-card-header">
                  <h3 className="adp-card-title">
                    📋 optimusLogger <span className="adp-card-src">/ems/sql?q=SELECT * FROM optimusLogger</span>
                  </h3>
                  <div className="adp-table-info">
                    <span className="adp-muted">
                      <strong>{(logPage - 1) * logPageSize + 1}–{Math.min(logPage * logPageSize, filteredLoggerData.length)}</strong> of <strong>{filteredLoggerData.length}</strong>
                    </span>
                    <select className="adp-select sm" value={logPageSize} onChange={(e) => { setLogPageSize(Number(e.target.value)); setLogPage(1); }}>
                  ...
                  <table className="adp-table">
                    ...
                    <tr>
                      {[
                        ...
                        <th key={col.k} style={{ width: col.w }} onClick={() => toggleLogSort(col.k)} className="adp-sortable">
                          {col.l} {logSortField === col.k && <span className="adp-sort-arrow">{logSortDir === 'asc' ? '▲' : '▼'}</span>}
                        </th>
                      ))}
                    </tr>
                    </thead>
                    ...
                    {pagedLogData.map((log, i) => (
                      <tr key={log.id} onClick={() => setLogDetailEntry(log)} className={i % 2 === 1 ? 'adp-row-alt' : ''}>
                        <td className="adp-td-muted">{log.id}</td>
                        <td className="adp-td-mono">{fmtDateTime(log.timestamp)}</td>
                    ...
                    {pagedLogData.length === 0 && (
                      <tr><td colSpan={6} className="adp-td-empty">
                        {loggerLoading ? 'Loading...' : 'No log entries match the current filters'}
                      </td></tr>
                    )}
                    </tbody>
                  </table>
                ...
                {logTotalPages > 1 && (
                  <div className="adp-pagination">
                    <button disabled={logPage === 1} onClick={() => setLogPage(1)}>⟨⟨</button>
                    <button disabled={logPage === 1} onClick={() => setLogPage((p) => Math.max(1, p - 1))}>⟨</button>
                    {Array.from({ length: Math.min(7, logTotalPages) }, (_, i) => {
                      let p: number;
                      ...
                      return (
                        <button key={p} className={logPage === p ? 'active' : ''} onClick={() => setLogPage(p)}>{p}</button>
                      );
                    })}
                    <button disabled={logPage === logTotalPages} onClick={() => setLogPage((p) => Math.min(logTotalPages, p + 1))}>⟩</button>
                    <button disabled={logPage === logTotalPages} onClick={() => setLogPage(logTotalPages)}>⟩⟩</button>
                  </div>
                )}
              </div>
            </div>
          ...
          {logDetailEntry && (
            <>
              <div className="adp-overlay" onClick={() => setLogDetailEntry(null)} />
              <div className="adp-modal">
                <div className="adp-modal-header">
                  <div className="adp-modal-title-row">
                    <span className="adp-level-badge" style={{ background: `${getLevelColor(logDetailEntry.level)}15`, color: getLevelColor(logDetailEntry.level) }}>{logDetailEntry.level}</span>
                    <span className="adp-modal-title">Log Entry #{logDetailEntry.id}</span>
                  </div>
                  <button className="adp-close-btn" onClick={() => setLogDetailEntry(null)}>✕</button>
                </div>
                <div className="adp-modal-body">
                  <div className="adp-detail-grid">
                    <span className="adp-detail-key">ID:</span><span className="adp-detail-val mono">{logDetailEntry.id}</span>
                    <span className="adp-detail-key">Timestamp:</span><span className="adp-detail-val mono">{fmtDateTime(logDetailEntry.timestamp)} UTC</span>
                    <span className="adp-detail-key">Date:</span><span className="adp-detail-val mono">{logDetailEntry.date}</span>
                    <span className="adp-detail-key">Hour:</span><span className="adp-detail-val mono">{logDetailEntry.hour}:00</span>
                    <span className="adp-detail-key">Level:</span>
                    <span className="adp-level-badge" style={{ background: `${getLevelColor(logDetailEntry.level)}15`, color: getLevelColor(logDetailEntry.level) }}>{logDetailEntry.level}</span>
                    <span className="adp-detail-key">Source:</span><code className="adp-code-inline">{logDetailEntry.source}</code>
                  </div>
                  <div className="adp-detail-section">
                    <span className="adp-detail-section-title">Message</span>
                    <p className="adp-detail-message">{logDetailEntry.message}</p>
                  </div>
                  <div className="adp-detail-section">
                    <span className="adp-detail-section-title">Raw JSON</span>
                    <pre className="adp-detail-json">{JSON.stringify(logDetailEntry, null, 2)}</pre>
                  </div>
                </div>
                <div className="adp-modal-footer">
                  <button className="adp-btn-sm" onClick={() => navigator.clipboard.writeText(JSON.stringify(logDetailEntry, null, 2))}>📋 Copy JSON</button>
                  <button className="adp-btn-sm" onClick={() => navigator.clipboard.writeText(logDetailEntry.message)}>📝 Copy Message</button>
                  <span style={{ flex: 1 }} />
                  <button className="adp-btn-primary" onClick={() => setLogDetailEntry(null)}>Close</button>
                </div>
              </div>
            </>
          )}
          ...
          {activeTab === 'ems-logs' && (
            <div className="adp-tab-body">
              <div className="adp-card">
                <div className="adp-card-header">
                  <h3 className="adp-card-title">📄 EMS Filtered Logs <span className="adp-card-src">GET /ems/logs?limit=100&since_min=120</span></h3>
                  <div className="adp-filter-chips">
                    {['ALL', 'INFO', 'WARN', 'ERROR'].map((lv) => (
                      <button key={lv} className={`adp-chip ${emsLogLevelFilter === lv ? 'active' : ''}`}
                              style={emsLogLevelFilter === lv ? { borderColor: getLevelColor(lv), background: `${getLevelColor(lv)}15`, color: getLevelColor(lv) } : {}}
                              onClick={() => setEmsLogLevelFilter(lv)}>
                        {lv} {lv !== 'ALL' && <span className="adp-chip-count">({emsLogLevelCounts[lv] || 0})</span>}
                      </button>
                    ))}
                ...
                <div className="adp-table-wrap">
                  <table className="adp-table">
                    ...
                    {filteredEmsLogs.map((log, i) => (
                      <tr key={i} className={i % 2 === 1 ? 'adp-row-alt' : ''}>
                        <td className="adp-td-mono">{fmtTime(log.timestamp)}</td>
                        <td>
                          <span className="adp-level-badge" style={{ background: `${getLevelColor(log.level)}15`, color: getLevelColor(log.level) }}>{log.level}</span>
                        </td>
                        <td className="adp-td-source">{log.source}</td>
                    ...
                    {filteredEmsLogs.length === 0 && (
                      <tr><td colSpan={4} className="adp-td-empty">
                        {emsLogsLoading ? 'Loading...' : 'No EMS logs available'}
                      </td></tr>
                    )}
                    </tbody>
                  </table>
                ...
                <div className="adp-card-footer">
                  <span className="adp-muted">Showing {filteredEmsLogs.length} of {emsLogs.length} entries</span>
                  <button className="adp-btn-sm" onClick={() => exportLogData('json')}>📥 Export</button>
                </div>
              </div>
            </div>
          )}
          ...
          {activeTab === 'events' && (
            <div className="adp-tab-body">
              <div className="adp-card">
                <div className="adp-card-header">
                  <h3 className="adp-card-title">⚡ System Events <span className="adp-card-src">GET /ems/events?limit=50&since_min=60</span></h3>
                ...
                <div className="adp-timeline">
                  {emsEvents.map((ev, i) => (
                    <div key={i} className="adp-timeline-item">
                      <div className="adp-timeline-track">
                        <div className="adp-timeline-dot" style={{ background: getEventColor(ev.type), boxShadow: `0 0 0 3px ${getEventColor(ev.type)}30` }} />
                        {i < emsEvents.length - 1 && <div className="adp-timeline-line" />}
                      </div>
                      <div className="adp-timeline-content">
                        <div className="adp-timeline-top">
                          <span className="adp-level-badge" style={{ background: `${getEventColor(ev.type)}15`, color: getEventColor(ev.type) }}>{ev.type}</span>
                          <span className="adp-muted">{fmtRelative(ev.timestamp)}</span>
                        </div>
                  ...
                  {emsEvents.length === 0 && (
                    <div className="adp-empty">{emsEventsLoading ? 'Loading events...' : 'No events available'}</div>
                  )}
                </div>
              </div>
            </div>
          ...
          {activeTab === 'performance' && (
            <div className="adp-tab-body">
              ...
              <div className="adp-kpi-row">
                {[
                  { l: 'Ops/Second', v: benchmarks ? String(benchmarks.ops_per_second) : 'N/A', c: '#7c3aed', s: 'Sustained throughput' },
                  { l: 'Avg Latency', v: benchmarks ? `${benchmarks.avg_latency_ms}ms` : 'N/A', c: '#2563eb', s: 'Mean response' },
                  { l: 'P99 Latency', v: benchmarks ? `${benchmarks.p99_latency_ms}ms` : 'N/A', c: '#d97706', s: '99th percentile' },
                  { l: 'Total Ops', v: benchmarks ? benchmarks.total_operations.toLocaleString() : 'N/A', c: '#0d9488', s: 'Since reset' },
                ].map((m, i) => (
                  <div key={i} className="adp-kpi-card">
              ...
              <div className="adp-card">
                <div className="adp-card-header">
                  <h3 className="adp-card-title">🚀 Query Strategy Performance <span className="adp-card-src">GET /benchmarks</span></h3>
                  {perfLoading && <span className="adp-spinner-sm" />}
                </div>
                <div className="adp-strategy-list">
                  {(benchmarks?.by_strategy || []).map((s, i) => {
                    const maxOps = Math.max(...(benchmarks?.by_strategy || []).map((x) => x.ops), 1);
                    const pct = (s.ops / maxOps * 100).toFixed(0);
                    const barColor = s.avg_ms < 10 ? '#16a34a' : s.avg_ms < 30 ? '#2563eb' : s.avg_ms < 50 ? '#d97706' : '#dc2626';
                    return (
                      <div key={i} className="adp-strategy-row">
                        <code className="adp-strategy-name">{s.strategy}</code>
                        <div className="adp-strategy-bar-track">
                          <div className="adp-strategy-bar-fill" style={{ width: `${pct}%`, background: barColor }}>
                            {parseInt(pct) > 30 && <span className="adp-bar-label">{s.ops} ops/s</span>}
                          </div>
                        </div>
                        <span className="adp-strategy-latency" style={{ color: barColor }}>{s.avg_ms}ms</span>
                      </div>
                    );
                  })}
                  {(!benchmarks || benchmarks.by_strategy.length === 0) && (
                    <div className="adp-empty">{perfLoading ? 'Loading...' : 'No benchmark data available'}</div>
                  )}
                </div>
              </div>
              ...
              <div className="adp-card">
                <div className="adp-card-header">
                  <h3 className="adp-card-title">🎯 Cache Statistics <span className="adp-card-src">via POST /command</span></h3>
                </div>
                <div className="adp-cache-grid">
                  {cacheStats ? [
                    { l: 'Hits', v: cacheStats.hits.toLocaleString(), c: '#16a34a' },
                    { l: 'Misses', v: cacheStats.misses.toLocaleString(), c: '#dc2626' },
                    { l: 'Ratio', v: `${(cacheStats.hit_ratio * 100).toFixed(1)}%`, c: '#0d9488' },
                    { l: 'Evictions', v: cacheStats.evictions.toLocaleString(), c: '#d97706' },
                    ...
                    <div key={i} className="adp-cache-item">
                      ...
                      <div className="adp-cache-value" style={{ color: m.c }}>{m.v}</div>
                    </div>
                  )) : (
                ...
                <div className="adp-table-wrap">
                  <table className="adp-table">
                    ...
                    {allStoreNames.map((storeName, i) => {
                      ...
                      return (
                        <tr key={storeName} className={i % 2 === 1 ? 'adp-row-alt' : ''}>
                          ...
                          {meshData.map((n) => (
                            <td key={n.nodeId} className="adp-td-mono bold">{n.stores[storeName]?.entry_count ?? '–'}</td>
                          ))}
                          <td className={`adp-td-mono bold ${synced ? 'adp-green' : 'adp-amber'}`}>±{delta}</td>
                          <td>
                              <span className={`adp-sync-badge ${synced ? 'synced' : 'syncing'}`}>
                    ...
                    {allStoreNames.length === 0 && (
                      <tr><td colSpan={3 + meshData.length} className="adp-td-empty">
                        {storesLoading ? 'Loading...' : 'No OptimusDB-CRUD store data available'}
                      </td></tr>
                    )}
                    </tbody>
                  </table>
          ...
          {activeTab === 'peers' && (
            <div className="adp-tab-content adp-fade-in">
              ...
              <div className="adp-kpi-row">
                ...
                <div className="adp-kpi-card">
                  ...
                  <div className="adp-kpi-value adp-green">
                    {peerData.reduce((sum, nd) => sum + nd.peers.filter((p) => p.connected).length, 0)}
                  </div>
                </div>
                <div className="adp-kpi-card">
                ...
                <div className="adp-card-body">
                  {peerData.length > 0 ? (
                      ...
                      <table className="adp-table">
                        ...
                        {peerData.map((nd) => (
                          <tr key={nd.nodeId} className="adp-tr">
                            ...
                            <td className="adp-td adp-mono" style={{ fontSize: '11px', maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                              {nd.peerId || '—'}
                            </td>
                            <td className="adp-td">{nd.peers.length}</td>
                            <td className="adp-td">
                                <span className={nd.peers.filter((p) => p.connected).length === nd.peers.length ? 'adp-green' : 'adp-amber'}>
                                  {nd.peers.filter((p) => p.connected).length}/{nd.peers.length}
                                </span>
                            </td>
                            <td className="adp-td">
                              <div className="adp-peer-chips">
                                {nd.peers.map((p, i) => (
                                  <span
                                    key={i}
                                    className={`adp-peer-chip ${p.connected ? 'connected' : 'disconnected'}`}
                                    title={`${p.peer_id}\n${p.addrs.join(', ')}`}
                                  >
                                      {p.connected ? '●' : '○'} {p.agent_name || p.peer_id.slice(0, 12) + '…'}
                                    {p.latency_ms !== undefined && (
                                      <span className="adp-peer-latency">{p.latency_ms}ms</span>
                                    )}
                                    </span>
                                ))}
                    ...
                    <div className="adp-empty-state">
                      {peersLoading ? 'Discovering peers…' : 'No peer data available'}
                    </div>
                  )}
                </div>
              </div>
                    ...
                    <div key={nd.nodeId} className="adp-addr-block">
                      ...
                      {nd.peers.map((p, i) => (
                        <div key={i} className="adp-addr-row">
                          <span className={`adp-addr-dot ${p.connected ? 'green' : 'red'}`}>●</span>
                          <span className="adp-mono" style={{ fontSize: '11px' }}>
                            {p.addrs.length > 0 ? p.addrs[0] : p.peer_id}
                          </span>
                          {p.addrs.length > 1 && (
                            <span className="adp-addr-more">+{p.addrs.length - 1} more</span>
                          )}
                        </div>
                      ))}
                      {nd.peers.length === 0 && <div className="adp-muted" style={{ padding: '4px 0 4px 16px' }}>No peers</div>}
                    </div>
                  ))}
                </div>
              </div>
              ...
              {contributionData.length > 0 && (
                <div className="adp-card">
                  <div className="adp-card-header">
                    <span className="adp-card-title">🏆 Node Contributions</span>
                  </div>
                  <div className="adp-card-body">
                    <div className="adp-table-wrap">
                      <table className="adp-table">
                              ...
                              <td className="adp-td">
                                <div className="adp-score-bar-wrap">
                                  <div
                                    ...
                                    style={{
                                      width: `${Math.min(100, (cd.score / Math.max(...contributionData.map((c) => c.score || 1))) * 100)}%`,
                                    }}
                                  />
            ...
            <div className="adp-tab-content adp-fade-in">
              ...
              <div className="adp-kpi-row">
                ...
                <div className="adp-kpi-card">
                  ...
                  <div className="adp-kpi-value adp-green">{credActiveCount}</div>
                </div>
                <div className="adp-kpi-card">
                  <div className="adp-kpi-label">Revoked</div>
                  <div className="adp-kpi-value adp-red">{credRevokedCount}</div>
                </div>
                <div className="adp-kpi-card">
              ...
              <div className="adp-card">
                ...
                <div className="adp-filter-row">
                  <div className="adp-filter-chips">
                    {credTypes.map((t) => (
                      <button
                        key={t}
                        className={`adp-chip ${credTypeFilter === t ? 'active' : ''}`}
                        onClick={() => setCredTypeFilter(t)}
                      >
                        {t === 'ALL' ? 'All Types' : t.replace('VerifiableCredential', 'VC')}
                        {t === 'ALL' ? ` (${credentials.length})` : ` (${credentials.filter((c) => c.type.includes(t)).length})`}
                      </button>
                    ))}
                  </div>
                  <input
                ...
                <div className="adp-card-body">
                  {filteredCreds.length > 0 ? (
                    <div className="adp-table-wrap">
                      <table className="adp-table">
                        ...
                        {filteredCreds.map((cred, idx) => (
                          ...
                          >
                            <td className="adp-td adp-mono" style={{ fontSize: '11px', maxWidth: '220px', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                              {cred.id}
                            </td>
                            <td className="adp-td">
                              {cred.type.map((t) => (
                                <span key={t} className="adp-cred-type-badge">{t.replace('VerifiableCredential', 'VC')}</span>
                              ))}
                            </td>
                            <td className="adp-td adp-mono" style={{ fontSize: '11px', maxWidth: '180px', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                              {cred.issuer}
                            </td>
                            <td className="adp-td adp-mono" style={{ fontSize: '11px', maxWidth: '180px', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                              {cred.credentialSubject?.id || cred.credentialSubject?.name || '—'}
                            </td>
                            <td className="adp-td">{cred.issuanceDate ? fmtTime(cred.issuanceDate) : '—'}</td>
                            <td className="adp-td">
                              {cred.revoked
                                ? <span className="adp-status-badge revoked">✗ Revoked</span>
                                : <span className="adp-status-badge active">✓ Active</span>
                              }
                            </td>
                          </tr>
                        ))}
                    ...
                    <div className="adp-empty-state">
                      {credentialsLoading ? 'Loading credentials…' : 'No credentials found'}
                    </div>
                  )}
                </div>
              </div>
                  ...
                  <div className="adp-modal" onClick={(e) => e.stopPropagation()}>
                    <div className="adp-modal-header">
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          <span className={`adp-status-badge ${credDetailEntry.revoked ? 'revoked' : 'active'}`}>
                    ...
                    <div className="adp-modal-body">
                      <div className="adp-detail-grid">
                        ...
                        {credDetailEntry.expirationDate && (
                          <div className="adp-detail-field"><span className="adp-detail-label">Expires</span><span>{credDetailEntry.expirationDate}</span></div>
                        )}
                      </div>
                      <div className="adp-detail-section">
                        <div className="adp-detail-label">Credential Subject</div>
                        <pre className="adp-json-block">{JSON.stringify(credDetailEntry.credentialSubject, null, 2)}</pre>
                      </div>
                      {credDetailEntry.proof && (
                        <div className="adp-detail-section">
                          <div className="adp-detail-label">Proof</div>
                          <pre className="adp-json-block">{JSON.stringify(credDetailEntry.proof, null, 2)}</pre>
                        </div>
                      )}
                      <div className="adp-detail-section">
                        <div className="adp-detail-label">Raw JSON</div>
                        <pre className="adp-json-block">{JSON.stringify(credDetailEntry, null, 2)}</pre>
                      </div>
                    </div>
                    <div className="adp-modal-footer">
                      <button className="adp-btn adp-btn-sm" onClick={() => { navigator.clipboard.writeText(JSON.stringify(credDetailEntry, null, 2)); }}>
                        📋 Copy JSON
                      </button>
            ...
            <div className="adp-tab-content adp-fade-in">
                  ...
                  {metadataHealth ? (
                    <div className="adp-health-grid">
                      <div className="adp-health-item">
                        ...
                        <span className={`adp-health-value ${metadataHealth.status === 'healthy' || metadataHealth.status === 'ok' ? 'adp-green' : 'adp-red'}`}>
                          {metadataHealth.status === 'healthy' || metadataHealth.status === 'ok' ? '● Healthy' : `● ${metadataHealth.status}`}
                        </span>
                      </div>
                      <div className="adp-health-item">
                        <span className="adp-health-label">Version</span>
                        <span className="adp-health-value adp-mono">{metadataHealth.version || '—'}</span>
                      </div>
                      <div className="adp-health-item">
                        <span className="adp-health-label">Uptime</span>
                        <span className="adp-health-value">{metadataHealth.uptime || '—'}</span>
                      </div>
                      <div className="adp-health-item">
                        <span className="adp-health-label">OptimusDB</span>
                        <span className={`adp-health-value ${metadataHealth.optimusdb_connected ? 'adp-green' : 'adp-red'}`}>
                          {metadataHealth.optimusdb_connected ? '● Connected' : '● Disconnected'}
                        </span>
                      </div>
                      <div className="adp-health-item">
                        <span className="adp-health-label">Cache Size</span>
                        <span className="adp-health-value">{metadataHealth.cache_size}</span>
                      </div>
                      <div className="adp-health-item">
                        <span className="adp-health-label">AI Model</span>
                        <span className={`adp-health-value ${metadataHealth.ai_model_loaded ? 'adp-green' : 'adp-amber'}`}>
                          {metadataHealth.ai_model_loaded ? '● Loaded' : '○ Not Loaded'}
                        </span>
                      </div>
                    </div>
                  ) : (
                    <div className="adp-empty-state">
                      {metadataLoading ? 'Loading metadata health…' : 'Metadata service not responding'}
                    </div>
                  )}
                </div>
              </div>
                ...
                <>
                  <div className="adp-kpi-row">
                    <div className="adp-kpi-card">
                      ...
                      <div className="adp-kpi-value">{metadataMetrics.total_enrichments.toLocaleString()}</div>
                    </div>
                    <div className="adp-kpi-card">
                      <div className="adp-kpi-label">Total Profiles</div>
                      <div className="adp-kpi-value">{metadataMetrics.total_profiles.toLocaleString()}</div>
                    </div>
                    <div className="adp-kpi-card">
                      <div className="adp-kpi-label">Avg Enrichment</div>
                      <div className="adp-kpi-value">{metadataMetrics.avg_enrichment_ms.toFixed(1)}<span className="adp-kpi-unit">ms</span></div>
                    </div>
                    <div className="adp-kpi-card">
                      <div className="adp-kpi-label">Databases</div>
                      <div className="adp-kpi-value">{metadataMetrics.databases_tracked}</div>
                    </div>
                    <div className="adp-kpi-card">
                      <div className="adp-kpi-label">Tables</div>
                      <div className="adp-kpi-value">{metadataMetrics.tables_tracked}</div>
                    </div>
                  </div>
                  ...
                  <div className="adp-card">
                    <div className="adp-card-header">
                      <span className="adp-card-title">💾 Metadata Cache Performance</span>
                    </div>
                    <div className="adp-card-body">
                      <div className="adp-health-grid">
                        <div className="adp-health-item">
                          <span className="adp-health-label">Cache Hits</span>
                          <span className="adp-health-value adp-green">{metadataMetrics.cache_hits.toLocaleString()}</span>
                        </div>
                        <div className="adp-health-item">
                          <span className="adp-health-label">Cache Misses</span>
                          <span className="adp-health-value adp-red">{metadataMetrics.cache_misses.toLocaleString()}</span>
                        </div>
                        <div className="adp-health-item">
                        ...
                        <div className="adp-health-item">
                          <span className="adp-health-label">Last Enrichment</span>
                          <span className="adp-health-value">
                            {metadataMetrics.last_enrichment ? fmtRelative(metadataMetrics.last_enrichment) : '—'}
                          </span>
                        </div>
                      </div>
                      {/* Cache hit ratio bar */}
                      <div className="adp-cache-ratio-bar" style={{ marginTop: '16px' }}>
                        <div className="adp-cache-ratio-label">
                          Hit Ratio: {(metadataMetrics.cache_hit_ratio * 100).toFixed(1)}%
                        </div>
                        <div className="adp-ratio-track">
                          <div
                            className="adp-ratio-fill"
                            style={{ width: `${Math.min(100, metadataMetrics.cache_hit_ratio * 100)}%` }}
                          />
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="adp-card">
                    <div className="adp-card-header">
                      <span className="adp-card-title">🤖 AI Enrichment Pipeline</span>
                    </div>
                    <div className="adp-card-body">
                      <div className="adp-health-grid">
                        <div className="adp-health-item">
                          <span className="adp-health-label">Datasets Enriched</span>
                          <span className="adp-health-value">{metadataMetrics.total_enrichments.toLocaleString()}</span>
                        </div>
                        <div className="adp-health-item">
                          <span className="adp-health-label">Profiles Generated</span>
                          <span className="adp-health-value">{metadataMetrics.total_profiles.toLocaleString()}</span>
                        </div>
                        <div className="adp-health-item">
                          <span className="adp-health-label">Avg Processing</span>
                          <span className="adp-health-value">{metadataMetrics.avg_enrichment_ms.toFixed(1)} ms</span>
                        </div>
                        <div className="adp-health-item">
                          <span className="adp-health-label">Model Status</span>
                          <span className={`adp-health-value ${metadataHealth?.ai_model_loaded ? 'adp-green' : 'adp-amber'}`}>
                            {metadataHealth?.ai_model_loaded ? 'TinyLlama Active' : 'Model Offline'}
                          </span>
                        </div>
                      </div>
                    </div>
              ...
              {!metadataMetrics && !metadataLoading && (
                <div className="adp-card">
                  <div className="adp-card-body">
                    <div className="adp-empty-state">
                      Metadata metrics not available. The metadata service may not be running or accessible.
                    </div>
                  </div>
                </div>
              )}
        ...
        <div className="adp-statusbar">
          <span className="adp-status-item">📊 {TABS.find((t) => t.id === activeTab)?.label}</span>
          <span className="adp-status-item">
            Cluster: <strong className="adp-green">{healthyCount}/{agents.length} Healthy</strong>
          </span>
          <span className="adp-status-item">
            Leader: <strong>{leader ? `${leader.name} ⭐ Term #${leader.electionTerm}` : 'N/A'}</strong>
          </span>
          {activeTab === 'logging' && (
            <span className="adp-status-item">
              Logs: <strong>{filteredLoggerData.length}</strong>/{loggerData.length}
              {hasLogFilters && <span className="adp-filtered-badge">• Filtered</span>}
            </span>
          )}
          <span className="adp-status-item">{autoRefresh ? '🔄 Live 30s' : '⏸ Paused'}</span>
          <span className="adp-status-src">
            {activeTab === 'logging' ? 'GET /ems/sql?q=SELECT * FROM optimusLogger' :
              activeTab === 'ems-logs' ? 'GET /ems/logs' :
                activeTab === 'events' ? 'GET /ems/events' :
                  activeTab === 'performance' ? 'GET /benchmarks · POST /command' :
                    activeTab === 'stores' ? 'GET /debug/optimusdb/mesh · /ems/sql' :
                      activeTab === 'peers' ? 'GET /peers · POST /command (contri)' :
                        activeTab === 'credentials' ? 'GET /credentials' :
                          activeTab === 'metadata' ? 'GET /api/v1/metadata/metrics · /health' :
