...
const QueryWorkbenchPage: React.FC = () => {
  ...
  const loadSchema = async () => {
    ...
    try {
      // ⭐ Use actual API format for schema loading
      const response = await axios.post(
        `${settings.apiBaseUrl}/${connection.context}/command`,
        {
          method: {
            argcnt: 2,
            cmd: 'sqldml'
  ...
  // ⭐ MAIN EXECUTE QUERY - MATCHES YOUR ACTUAL API FORMAT
  const executeQuery = useCallback(async () => {
    ...
    try {
      const endpoint = `${settings.apiBaseUrl}/${connection.context}/command`;
  ...
  }, [query, connection, settings, history]);
