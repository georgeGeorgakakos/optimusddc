...
```bash
$ docker pull amundsendev/optimusddc-search:latest
$ docker run -p 5001:5001 amundsendev/optimusddc-search
# - alternative, for production environment with Gunicorn (see its homepage link below)
$ ## docker run -p 5001:5001 amundsendev/optimusddc-search gunicorn --bind 0.0.0.0:5001 search_service.search_wsgi
