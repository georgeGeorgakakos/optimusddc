services:
  ...
  catalogsearch:
    build:
      context: ./search
      dockerfile: Dockerfile.search.public    # adjust if your search Dockerfile.metadata.local has a different name
    ...
    networks:
      - catalognet
    environment:
      - PROXY_ENDPOINT=elastic_ddc
   # command: gunicorn -w 2 --bind :5013 search_service.search_wsgi

  catalogmetadata:
    build:
      context: ./metadata
      dockerfile: Dockerfile.metadata.local    # adjust if needed
    ...
    command: gunicorn -w 2 --bind :5014 metadata_service.metadata_wsgi

  catalogfrontend:
    build:
      context: ./frontend
      dockerfile: Dockerfile.frontend.local    # adjust if your frontend Dockerfile.metadata.local is elsewhere
    ...
    #command: gunicorn -w 2 --bind :5015 amundsen_application.wsgi
